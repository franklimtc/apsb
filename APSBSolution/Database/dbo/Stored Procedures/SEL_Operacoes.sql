...
AS BEGIN            
 ...
 , r.cdEmissao            
 , r.cdPagamento            
 , CASE WHEN vwR.status = 'R' THEN vwR.dtRepasse ELSE NULL END [cdRepasse]             
 , CASE       
 WHEN ISNULL(vwR.cvValorRepassado,0) > ISNULL(r.cvValorPago,0) OR ISNULL(r.cvValorPago,0) > ISNULL(r.cvValor,0)  THEN 4      
 ...
 WHERE r.cbStatus = @cbStatus AND r.cbArquivado=@cbArquivado          
    AND   (r.cdEmissao BETWEEN ISNULL(@dtInicio, DATEADD(year, -10, GETDATE())) AND  ISNULL(@dtFim, DATEADD(year, +10, GETDATE()))  OR cdEmissao IS NULL)  
 UNION            
