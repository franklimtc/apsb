...
IF @idReceita IS NOT NULL     
 BEGIN    
	...
	INNER JOIN tbClinicaProfissional cp ON cp.idClinica = re.IdClinica AND cp.idProfissional = p.IdProfissional
	LEFT JOIN tbObservacoes O on cp.IdObservacao = o.IdObservacao           
	WHERE r.ccStatus <> 'C' AND r.idReceita = @idReceita          
 END    
 ELSE    
 BEGIN    
  SELECT  c.ccApelido, p.ccNome, r.cvNF, r.cdEmissao, CAST(r.cvValor AS DECIMAL(10,2)) ValorNF
  , CAST(r.cvValorPago AS DECIMAL(10,2)) ValorPago, pr.dataRepasse cdRepasse, CAST(pr.cvValor - (pr.cvValor * (ISNULL(pr.cvTaxaProfissional,0)/100)) AS DECIMAL(10,2))  ValorRepasse      
  FROM tbReceitas r    
