...
IF @idReceita IS NOT NULL 
	BEGIN
		...
		, r.IdProfissional          
		, p.ccNome    
		, ISNULL(tcp.cvTaxaProfissional, 0) cvTaxaProfissional    
		, CAST(r.cvValor AS DECIMAL(10,2)) [cvValor]          
		, CAST(r.cvValor - (r.cvValor* ISNULL(tcp.cvTaxaProfissional, 0) / 100) AS DECIMAL(10,2)) [cvValorLiquido]          
		, r.ccStatus          
		, r.dataRepasse          
		, r.ccCriadoPor          
		, o.observacao          
	...
	ELSE
	BEGIN
		SELECT  c.ccApelido, p.ccNome, r.cvNF, r.cdEmissao, CAST(r.cvValor AS DECIMAL(10,2)) ValorNF, CAST(r.cvValorPago AS DECIMAL(10,2)) ValorPago, pr.dataRepasse cdRepasse, CAST(pr.cvValor - (pr.cvValor * (ISNULL(pr.cvTaxaProfissional,0)/100)) AS DECIMAL(10,2))  ValorRepasse  
		FROM tbReceitas r
